...
export class ReadingOrderProcessor {
  ...
  process(
    ...
    const validBlocks = textBlocks.filter(
      (b) =>
        b.confidence >= minConfidence && b.text && b.text.trim().length > 0,
    )
  ...
  private processXYCut(validBlocks: TextBlock[]): TextBlock[] {
    const rawBboxes = validBlocks.map((b) => [
      b.x,
      b.y,
      b.x + b.width,
      b.y + b.height,
    ...
    const result = validBlocks.map((block, i) => ({
      ...block,
      readingOrder: ranks[i] + 1,
    }))
  ...
  // ---------------------------------------------------------------------------
  private normalizeBboxes(bboxes: number[][]): {
    normBboxes: number[][]
    w: number
    h: number
  } {
  ...
  private makeMeshTable(bboxes: number[][], w: number, h: number): number[][] {
    const table: number[][] = Array.from({ length: h }, () =>
      new Array(w).fill(0),
    )
