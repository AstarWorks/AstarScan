...
export class LayoutDetector {
  ...
  async initialize(modelData: ArrayBuffer): Promise<void> {
    ...
    try {
      ...
      console.log(
        `Layout detector initialized: input ${this.inputSize.width}×${this.inputSize.height}`,
      )
    } catch (error) {
  ...
  async detect(
    ...
    console.log(
      `[LayoutDetector] ${lines.length} line regions, ${blocks.length} text blocks detected`,
    )
    return { lines, blocks }
  }
