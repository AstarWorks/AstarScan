...
export class TextRecognizer {
  ...
  private async loadConfig(): Promise<void> {
    if (this.isEuropean) {
      ...
      const EUROPEAN_VOCAB =
        '0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ!"#$%&\'()*+,-./:;<=>?@[\\]^_`{|}~°£€¥¢฿àâéèêëîïôùûüçÀÂÉÈÊËÎÏÔÙÛÜÇáãíóõúÁÃÍÓÕÚñÑ¡¿äößÄÖẞčďěňřšťůýžČĎĚŇŘŠŤŮÝŽąćęłńśźżĄĆĘŁŃŚŹŻìòÌÒæøåÆØÅ§'
      this.config.charList = EUROPEAN_VOCAB.split('')
      this.config.maxLength = 32
      console.log(
        `European character list loaded: ${this.config.charList.length} characters`,
      )
      return
    }
    ...
    if (yamlConfig?.text_recognition) {
      ...
      if (textConfig.input_shape)
        this.config.inputShape = textConfig.input_shape as [
          number,
          number,
          number,
          number,
    ...
    if ((yamlConfig?.model as Record<string, unknown>)?.charset_train) {
      ...
      console.log(
        `Character list loaded: ${this.config.charList.length} characters`,
      )
    }
  }
    ...
    try {
      ...
      const avgConfidence =
        filteredConfidences.length > 0
          ? filteredConfidences.reduce((a, b) => a + b, 0) /
            filteredConfidences.length
          : 0
