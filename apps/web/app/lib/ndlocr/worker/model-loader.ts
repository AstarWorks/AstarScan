...
const MODEL_BASE_URL =
  (import.meta.env.VITE_MODEL_BASE_URL as string | undefined) || '/models/ocr'
...
export async function loadModel(
  ...
  if (!modelUrl) {
    throw new Error(
      `Unknown model type: ${modelType} for language ${language ?? 'ja'}`,
    )
  }

  // キャッシュキーに言語プレフィックスを付けて衝突を防止
  const cacheKey =
    language && language !== 'ja' ? `${language}:${modelType}` : modelType
